...
•According to the OWASP, “Command injection (a.k.a shell injection) is
an attack in which the goal, is the execution of arbitrary commands
on the host operating system through a vulnerable application”
• This attack is possible when an application passes unsafe user supplied data
(i.e forms, cookies, HTTP headers etc) to a system shell.
...
• FULL_COMMAND+COMMAND_TERMINATOR (when the command is
directly injected into the beginning of the expression and then it is
appended with some arguments/other commands)
• COMMAND_SEPARATOR + FULL_COMMAND (when command is appended
as an argument of a command hardcoded in the expression)
